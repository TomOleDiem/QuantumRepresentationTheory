...
This blueprint lays out a roadmap from the bare notion of a Lie algebra
representation to the energy spectrum of the hydrogen atom, formalized on top
of Mathlib's existing Lie theory. It is organized as a dependency graph of
\emph{theorems and constructions}, not of individual small lemmas: each node
below corresponds to a genuine mathematical statement, and the graph edges
(via \texttt{uses}) record exactly which results feed into which.
...
\begin{definition}[Nondegenerate invariant bilinear form]
  ...
  radical is trivial. (For $L$ semisimple over $\mathbb{C}$, the Killing form
  is such a form.)
\end{definition}
...
\begin{definition}[Angular momentum representation]
  ...
  \cong \sltwo$ on a state space $V$, presented via self-adjoint operators
  $J_x, J_y, J_z$ satisfying $[J_x,J_y] = iJ_z$ and cyclic permutations.
\end{definition}
