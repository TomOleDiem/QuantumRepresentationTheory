...
special to $n=3$ (the only dimension in which $\Lambda^2 V\cong V$). The same
issue recurs one level up: \texttt{BoundStateRepresentation}
(\S\ref{sec:hyd-basic}) presents the Runge--Lenz vector as three
unconstrained operators $M_x,M_y,M_z$ with no commutation hypotheses at all
relating them to $\vec J$ or to each other, which is why
\texttt{Hydrogen/Symmetry.lean}'s $\vec I,\vec K$ commutator theorems
(\S\ref{sec:hyd-symmetry}) are currently unprovable as stated --- nothing in
the structure pins down the $\liealg{so}(4)$ relations a genuine Runge--Lenz
vector must satisfy. A single $\liealg{so}(4)$-antisymmetric-tensor
presentation of $(\vec J,\vec M)$ together would fix both problems at once
(and generalize past $n=3,4$ for free), at the cost of rederiving the
current per-component commutation-relation statements as theorems rather
than definitions. Not attempted in this pass.
