...
\begin{theorem}[Linear independence of the primitive-vector string]
  ...
  \leanok
  If $K$ has characteristic zero and $m$ is primitive of weight
  $n \in \mathbb{N}$, the vectors $m, fm, \dots, f^n m$ are linearly
  independent, since they lie in distinct $h$-eigenspaces
  (Theorem~\ref{thm:sl2-string-h}) and are all nonzero
  (Theorem~\ref{thm:sl2-string-finite}).

  \emph{The characteristic-zero hypothesis is necessary}, not a formalization
  artifact, and was missing from an earlier version of this statement: in
  characteristic $2$, the adjoint representation of $\sltwo$ on itself has a
  primitive vector $m = e$ of weight $n = 2$ (since $2 = 0$ in $K$), but
  $f^2 \cdot e = -2f = 0$, so the length-$3$ string $(e, \pm h, 0)$ is not
...
\begin{theorem}[Classification of jointly irreducible commuting $\sltwo$-actions]
  ...
  simultaneously. Then there exist $m, n \in \mathbb{N}$ with
  $\dim_K V = (m+1)(n+1)$ and a linear isomorphism
  \[
    V \;\cong_K\; V(m) \otimes_K V(n)
  \]
  as $K$-vector spaces. Proved via a joint primitive vector: since $\rho_1,
  \rho_2$ commute, a highest-weight vector for $\rho_1$ can be found inside a
  $\rho_1$-weight-space that is itself $\rho_2$-invariant, giving a single
  vector primitive for both triples; the bivariate string it generates is a
  basis of $V$ (linear independence via a combined eigenvalue, spanning via
  the joint-irreducibility hypothesis), giving the dimension count directly.
  As with Theorem~\ref{thm:sl2-clebsch-gordan}, the isomorphism does not yet
  intertwine $(\rho_1,\rho_2)$ with the actions of the two tensor factors
  (Theorem~\ref{thm:sl2-tensor-action}); that upgrade is future work.
\end{theorem}
...
\begin{theorem}[Clebsch--Gordan decomposition]
  ...
  \]
  as $K$-vector spaces, with matching dimension count $(m+1)(n+1) =
  \sum_{k=0}^{\min(m,n)} (m+n-2k+1)$. The vector-space isomorphism is proved
  directly from the dimension count (any two finite-dimensional spaces of
  equal dimension over a field are linearly isomorphic); it does not yet
  intertwine the diagonal $\sltwo$-action on the left with the direct-sum
  action on the right (Theorem~\ref{thm:sl2-tensor-action}), which would
  need the classification theorem (Theorem~\ref{thm:sl2-classification})
  and is future work.
\end{theorem}
...
\begin{definition}[Basis-dependent Casimir element]
  ...
  \uses{def:cas-dual-basis, def:rep-to-end, def:cas-invariant-form}
  \leanok
  For a representation $\rho$ of $L$ on $V$, a \emph{nondegenerate and
  Lie-invariant} form $\Phi$ (Definition~\ref{def:cas-invariant-form}), and a
  basis $x_1,\dots,x_d$ of $L$ with $\Phi$-dual basis $x^1,\dots,x^d$, the
  Casimir operator is
  \[
    C_\rho \;=\; \sum_{i=1}^d \rho(x_i)\,\rho(x^i) \;\in\; \End_K(V).
  \]
  Invariance of $\Phi$ is essential here, not just nondegeneracy: it is what
  makes Theorem~\ref{thm:cas-commutes-generators} true.
\end{definition}
...
\begin{theorem}[Schur gives a scalar Casimir]
  ...
  \leanok
  \uses{thm:rep-schur, thm:cas-commutes-generators, def:cas-casimir}
  ...
  scalar $c(\rho) \in K$. The Lean proof takes a direct route: package
  Theorem~\ref{thm:cas-commutes-generators} into a \texttt{LieModuleHom}
  self-map of $V$ and apply Theorem~\ref{thm:rep-schur} directly, rather than
  going through the universal-enveloping-algebra route of
  Theorem~\ref{thm:cas-central-intertwines}.
\end{theorem}
...
\begin{theorem}[Angular momentum generates an $\sltwo$-triple]
  ...
  If $J_z \neq 0$, then $(J_z, J_+, J_-)$ is an $\sltwo$-triple in the sense
  of Definition~\ref{def:sl2-triple}, via the bridge construction of
  Remark~\ref{def:rep-bridge} turning $\rho$ into local Lie-module instances.

  \emph{The hypothesis $J_z \neq 0$ is necessary}, not a formalization
  artifact: the all-zero representation ($J_x=J_y=J_z=0$) satisfies
  Definition~\ref{def:am-representation} but gives $h = 2J_z = 0$, which an
  $\sltwo$-triple explicitly excludes. This is the honest statement of a real
  fact --- the trivial, spin-$0$ representation does not generate an actual
  copy of $\sltwo$ inside $\operatorname{End}(V)$ --- and it means
  Theorem~\ref{thm:am-irreducibility-transfer} and
  Theorem~\ref{thm:am-spin-classification} downstream will need to either
  carry this hypothesis or special-case spin $0$ separately.
\end{theorem}
...
\begin{definition}[Bound-state representation]
  ...
  the (unnormalized) Runge--Lenz operator $\vec M$, subject to the standard
  quantum Runge--Lenz algebra: $\vec M$ transforms as a vector under $\vec
  J$ ($[J_i,M_j]=i\varepsilon_{ijk}M_k$) and its self-commutator closes back
  onto $\vec J$, rescaled by the fixed energy eigenvalue
  ($[M_i,M_j]=i\varepsilon_{ijk}(-2E/m)J_k$). These relations are genuine
  data about the representation (not derivable from
  Definition~\ref{def:am-representation} alone) and are taken as given here,
  consistent with this project's algebraic scope; without them
  $\vec M \equiv 0$ would be a valid but physically vacuous instance, which
  is why an earlier version of this definition (omitting them) left
  Theorem~\ref{thm:hyd-two-factors-commute} unprovable.
\end{definition}
...
things worth knowing about the current state of Mathlib relevant to this
blueprint: gaps this project fills, gaps it doesn't, and available
infrastructure it doesn't yet use.
...
\paragraph{Angular momentum as a 3-vector is a low-dimension coincidence,
not the right general object.} \texttt{AngularMomentumRepresentation}
(\S\ref{sec:am-basic}) presents angular momentum as three operators
$J_x,J_y,J_z$ with axiomatized commutation relations, mirroring the
physicist's $\vec J$. The correct general object is the antisymmetric
generator $J_{ij}=-J_{ji}$ of rotations in the $(i,j)$-plane --- an element
of $\liealg{so}(n)$, i.e.\ of $\Lambda^2 V$ for the defining representation
--- and identifying it with a vector via $J_i = \varepsilon_{ijk}J_{jk}$ is
special to $n=3$ (the only dimension in which $\Lambda^2 V\cong V$). A
general $\liealg{so}(n)$-antisymmetric-tensor presentation would generalize
past $n=3$ for free, at the cost of rederiving the current per-component
commutation-relation statements as theorems rather than definitions; not
attempted in this pass, since it is a genuinely different (larger) scope
than fixing the concrete gap below.

The same issue recurred one level up, and \emph{was} fixed (in the concrete,
$n=3$-specific form): an earlier version of \texttt{BoundStateRepresentation}
(\S\ref{sec:hyd-basic}) presented the Runge--Lenz vector as three
unconstrained operators $M_x,M_y,M_z$ with no commutation hypotheses at all
relating them to $\vec J$ or to each other, leaving
\texttt{Hydrogen/Symmetry.lean}'s $\vec I,\vec K$ commutator theorems
(\S\ref{sec:hyd-symmetry}) unprovable as stated (concrete counterexample:
$\vec M \equiv 0$). Definition~\ref{def:hyd-bound-state} now includes the
standard quantum Runge--Lenz commutation relations
($[J_i,M_j]=i\varepsilon_{ijk}M_k$, $[M_i,M_j]=i\varepsilon_{ijk}(-2E/m)J_k$,
spelled out component-wise rather than via the $\liealg{so}(n)$-tensor
machinery above) as explicit hypotheses, and
Theorem~\ref{thm:hyd-two-factors-commute} is proved from them. This is the
scoped, $n=3,4$-specific fix; the fully general $\liealg{so}(n)$ rewrite
described above remains future work.
